import { Order } from "../../codegen/api/fetch/api";
import {
  vnp_HashSecret,
  vnp_TmnCode,
  vnp_Url,
...
module.exports = function (Order: PersistedModelStatic<Order>) {
  (Order as any).addToCart = async function (ctx: any) {
  ...
  Order.afterRemote("find", async (ctx: any) => {
    try {
      const pg = knex("order").count().toString();
      const data = await executeNativeSql(
        Order.app.dataSources.postgres.connector,
        pg,
        []
      );
  ...
  (Order as any).statisticByAmount = async function (ctx: any) {
  ...
  (Order as any).statisticByRevenue = async function (ctx: any) {
    const year = ctx.req.query.year;
    let dayOfFeburary = 0;
    if (year % 4 == 0) {
      dayOfFeburary = 29;
  ...
  (Order as any).paymentOrder = async function (ctx: any) {
    const { orderType, amount, orderDescription, bankCode, language, idOrder } =
      ctx.req.body;
    const tmnCode = vnp_TmnCode;
    const secretKey = vnp_HashSecret;
    let vnpUrl = vnp_Url;
    const returnUrl = `${web_Url}payment-success?orderId=${idOrder}`;
    const ipAddr =
